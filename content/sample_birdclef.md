...
- Best model reached **0.847** macro F1 score
- Inference time: ~50ms per audio clip on a single GPU

## Lessons Learned

1. Bird audio is highly variable — species sound very different even within the same family
2. Augmentation is critical; without it, the model overfits to dominant species
3. The transformer attention layers significantly improved recall for rare species
